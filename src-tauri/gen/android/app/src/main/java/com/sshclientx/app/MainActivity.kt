...
class MainActivity : TauriActivity() {
  // T113/FR-038: initializes ndk-context's global Android context inside
  // the Rust library (see src-tauri/src/android_bridge.rs for why this
  // has to happen here — neither Tauri nor its Android runtime do this on
  // their own) so the Android keystore backend (`keyring`'s
  // android-native-keyring-store) can be used later. TauriActivity's own
  // onCreate has already loaded the native library by the time super
  // returns, and the WebView that could dispatch a Tauri command to the
  // keystore hasn't started running any JS yet — so this always runs
  // first. NOTE: this file lives under gen/android/ and IS regenerated by
