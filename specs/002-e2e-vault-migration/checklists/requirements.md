...
3. *Scope bounded* — the four "must be in scope" items from the request are each
   ...
   US2/FR-019–022, restore-over-existing is decided *yes* (FR-032/033), Android is
   decided *stays refusing* (FR-038) with the format split flagged as Q2.
...
- **D2**: desktop creates and migrates; Android opens, uses, and saves a new-format vault
  whose key arrived via the recovery kit, but cannot create or migrate one. Added
  FR-038 – FR-042, SC-012, US2 scenarios 6–7, and three edge cases. The Android
  recovery-kit restore accepting a vault file is a carve-out introduced here, not present
  in the source request — without it the Android path is a dead end, since general import
  is refused on that platform.
