...
const QrGuestScanPanel = () => {
  ...
  const scan = async (text: string) => {
    if (!text.trim()) return;
    setBusy(true); setError(null); setTicket(text);
  ...
  const cancel = async () => {
    ...
    setScanned(null); setResult(null); setTicket(""); setError(null);
  };

  if (result) return <div className="flex-1 grid place-items-center p-5"><div className="max-w-md w-full rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center space-y-3"><CheckCircle2 size={30} className="mx-auto text-emerald-300" /><h2 className="text-lg font-bold text-white">Transfer complete</h2><p className="text-sm text-zinc-300">Profile <strong>{result.landed_profile_name}</strong> is ready on this device.</p><button onClick={cancel} className="h-10 px-5 rounded-lg bg-primary text-black text-sm font-semibold">Start another transfer</button></div></div>;
  if (scanned) return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-8"><div className="max-w-md mx-auto space-y-4">
      ...
      <label className="block space-y-1"><span className="text-[10px] uppercase tracking-wider text-zinc-500">New vault password, or existing profile password</span><input type="password" value={password} onChange={e => setPassword(e.target.value)} className={input} placeholder="At least 8 characters for a new device" /></label>
